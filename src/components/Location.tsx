...
import { Link, Loader2, AlertCircle, AlertTriangle, Search, X, ChevronUp, ChevronDown as ChevronDownIcon, ChevronsUpDown, Filter, Check, Columns2, Info, BookmarkPlus, Copy, Trash2, ExternalLink, Bookmark, Pencil, CheckCheck } from "lucide-react"
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 border rounded-xl overflow-hidden shadow-sm bg-background">
      ...
      <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 border-b bg-muted/40 shrink-0">
        ...
        {/* ── Read-only badge (shared view) / Links button ── */}
        {isSharedView ? (
          <span className="ml-auto flex items-center gap-1 h-6 px-2 rounded-full border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-[10px] font-semibold shrink-0">
            <Info className="size-3" />Read-only
          </span>
        ) : (
          <Button
            size="sm"
              ...
              {!generateConfirm ? (
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 h-7 px-3 rounded-lg bg-primary text-primary-foreground text-[12px] font-semibold hover:bg-primary/90 transition-colors shrink-0 disabled:opacity-50"
                  >
                    <Link className="size-3" />Generate
                  </button>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 pt-0.5">
                    ...
                    >
                      {isShortening ? <Loader2 className="size-3 animate-spin" /> : <CheckCheck className="size-3" />}
